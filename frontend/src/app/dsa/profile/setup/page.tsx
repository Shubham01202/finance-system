// Path: frontend/src/app/dsa/profile/setup/page.tsx
"use client";
...
/* ─────────────────────────────────────────────
   PAGE
───────────────────────────────────────────── */
export default function DSAProfileSetupPage() {
  const router = useRouter();

  const [formData, setFormData] = useState<DSAProfileForm>({
  ...
  useEffect(() => {
    ...
    // Dynamic states — pulled from the same admin-managed States table
    // your other forms use, filtered to active ones only.
    fetch(`${API}/api/catalog/states`)
      .then(r => r.json())
      .then(d => {
    ...
    fetch(`${API}/api/dsa/profile`, {
      ...
      .then(d => {
        ...
        if (data && (data.agency_name || data.dsa_code)) {
          setFormData({
            agency_name:      data.agency_name      || "",
            dsa_code:         data.dsa_code         || "",
            empanelment_date: data.empanelment_date ? data.empanelment_date.split("T")[0] : "",
  ...
  const handleSubmit = async () => {
    ...
    (Object.keys(formData) as (keyof DSAProfileForm)[]).forEach(key => {
    ...
    try {
      const res = await fetch(`${API}/api/dsa/profile`, {
        method: "POST",
      ...
      if (data.success) {
        if (isEditMode) {
          ...
          setTimeout(() => { router.push("/dsa/profile"); }, 1200);
        } else {
  ...
  return (
    <DSALayout s={s} userName={userName} handleLogout={handleLogout}>
      {pageLoading ? (
        ...
        <div className="dsa-setup-wrap">
            ...
            <div className="dsa-setup-header-inner" style={s.headerInner}>
              ...
              <div className="dsa-header-right" style={s.headerRight}>
                ...
                <div style={s.headerSub}>
                  {isEditMode
                    ? `Welcome back, ${userName.split(" ")[0]} — update your DSA profile below`
                    : `Welcome, ${userName.split(" ")[0]} — complete your DSA profile to continue`}
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="dsa-setup-container" style={s.container}>
            ...
            {isEditMode && (
              <button style={s.backBtn} onClick={() => router.push("/dsa/profile")}>
            ...
            <div style={s.noticeBanner}>
              ...
              <div>
                <div style={{ fontWeight: 700, fontSize: 14, color: "#1d4ed8" }}>
                  {isEditMode ? "Update DSA Profile" : "One-time DSA Profile Setup"}
                </div>
                <div style={{ fontSize: 13, color: "#3b82f6", marginTop: 3 }}>
                  Your DSA empanelment details are required to apply loans on behalf of clients. This is verified and stored securely.
                </div>
              </div>
            </div>
            ...
            <div className="dsa-setup-card" style={s.card}>
              ...
              {/* ── AGENCY DETAILS ── */}
              <Section title="Agency Details" icon="🏢" subtitle="Your DSA agency information">
                <TwoCol>
                  <Field label="DSA Agency Name" required error={fieldErrors.agency_name}>
                    <InputEl
                      name="agency_name" placeholder="e.g. Sharma Loan Services"
                      value={formData.agency_name} onChange={handleChange}
                      icon={<FaBuilding />}
                    />
                  </Field>
                  <Field label="DSA Code" required error={fieldErrors.dsa_code}>
                    <InputEl
                      name="dsa_code" placeholder="e.g. DSA123456"
                      value={formData.dsa_code} onChange={handleChange}
                      icon={<FaIdCard />}
                    />
                  </Field>
                  <Field label="Date of Empanelment" required error={fieldErrors.empanelment_date}>
                    <InputEl
                      name="empanelment_date" type="date"
                      value={formData.empanelment_date} onChange={handleChange}
                      icon={<FaCalendarAlt />}
                    />
                  </Field>
                </TwoCol>
              ...
              {/* ── OFFICE ADDRESS ── */}
              <Section title="Office Address" icon="📍" subtitle="Your registered DSA office address">
              ...
              {/* ── CERTIFICATE UPLOAD ── */}
              <Section title="DSA Certificate / Empanelment Proof" icon="📄" subtitle="Upload your DSA empanelment or agreement certificate">
                ...
                <div style={{
                  ...
                  <div style={s.docCardTop}>
                    <div>
                      <div style={s.docLabel}>DSA Certificate / Empanelment Proof</div>
                      <div style={s.docDesc}>Empanelment letter or DSA agreement document</div>
                    </div>
                    {certificate && (
      ...
      <style jsx>{`
        .dsa-setup-wrap { margin: -32px -36px 0; }

        @media (max-width: 900px) {
          .dsa-setup-wrap { margin: -20px -16px 0; }
          .dsa-setup-header { padding: 18px 16px 44px !important; }
          .dsa-setup-header-inner { flex-direction: column; align-items: flex-start !important; }
          .dsa-header-right { text-align: left !important; }
          .dsa-setup-container { padding: 0 14px !important; }
          .dsa-setup-card { padding: 22px 18px !important; }
          .dsa-submit-row { flex-direction: column; align-items: stretch !important; }
          .dsa-submit-btn { width: 100%; justify-content: center; }
        }

        @media (max-width: 480px) {
          .dsa-setup-card { padding: 18px 14px !important; }
          .dsa-setup-header { padding: 16px 14px 40px !important; }
        }
      `}</style>
    </DSALayout>
  );
...
/* ─────────────────────────────────────────────
   STYLES — inline objects consumed by DSALayout/DSASidebar + this page
───────────────────────────────────────────── */
const s: Record<string, React.CSSProperties> = {
  ...
  noticeBanner: { display: "flex", alignItems: "flex-start", gap: 12, background: "#eff6ff", border: "1px solid #bfdbfe", borderRadius: 12, padding: "16px 20px", marginBottom: 16, boxShadow: "0 2px 8px rgba(0,0,0,0.05)" },
  ...
  input: { width: "100%", border: "1.5px solid #e5e7eb", borderRadius: 10, padding: "11px 14px", fontSize: 14, color: "#1e293b", background: "#f9fafb", outline: "none", boxSizing: "border-box" as const },
  select: { width: "100%", border: "1.5px solid #e5e7eb", borderRadius: 10, padding: "11px 14px", fontSize: 14, color: "#1e293b", background: "#f9fafb", outline: "none", appearance: "auto", boxSizing: "border-box" as const },
  ...
  btnSubmit: { display: "flex", alignItems: "center", justifyContent: "center", gap: 8, background: "linear-gradient(135deg,#1e3a5f,#2d5986)", color: "#fff", border: "none", padding: "13px 32px", borderRadius: 10, fontSize: 15, fontWeight: 700, cursor: "pointer" },
