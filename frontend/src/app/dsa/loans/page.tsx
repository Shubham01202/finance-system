// Path: frontend/src/app/dsa/loans/page.tsx
"use client";
...
/* ─────────────────────────────────────────────
   PAGE
───────────────────────────────────────────── */
export default function DSALoansPage() {
  ...
  const fetchLoans = useCallback(async (token?: string, filters?: Record<string, string>) => {
    ...
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/dsa/loans?${params.toString()}`, {
    ...
    <DSALayout s={s} userName={userName} handleLogout={handleLogout}>
      <div className="dsa-loans-wrap">
        ...
        <div className="dsa-topbar" style={s.topBar}>
          ...
          <button className="dsa-new-btn" style={s.newBtn} onClick={() => router.push("/dsa/apply")}>
            + New Application
          </button>
        </div>
        ...
        {/* ── SUMMARY STRIP ── */}
        <div className="dsa-summary-strip" style={s.summaryStrip}>
        ...
        {/* ── FILTER BAR ── */}
        <div className="dsa-filter-card" style={s.filterCard}>
          ...
          <div className="dsa-filter-grid" style={s.filterGrid}>
            {/* Search by name */}
            ...
            {/* Filter by partner DSA */}
            <FilterField label="Partner DSA Name">
              <div style={s.inputWrap}>
                <FaSearch style={s.inputIcon} />
                <input
          ...
          <div className="dsa-filter-actions" style={s.filterActions}>
        ...
        {/* ── LOANS TABLE ── */}
        <div className="dsa-table-card" style={s.card}>
          {loading ? (
            <div style={s.center}>
              <div style={s.spinner} />
            ...
            <div className="dsa-table-scroll" style={{ overflowX: "auto" }}>
              <table style={s.table}>
                <thead>
                  <tr>
                    <Th>#</Th>
                 ...
                 {paginatedLoans.map((loan, i) => {
                    ...
                    return (
                      <tr key={loan.id} style={s.trow}>
                        ...
                        <td style={s.tdBase}>
                          <div style={{ fontSize: 13, color: "#475569", fontWeight: 600 }}>{loan.dsa_name || "—"}</div>
                          <div style={{ fontSize: 11, color: "#94a3b8", marginTop: 2 }}>{loan.dsa_firm || ""}</div>
                        </td>
                        ...
                        <td style={s.tdBase}>
                          <div style={{ display: "flex", gap: 6 }}>
                            <button
                              onClick={() => router.push(`/dsa/loans/${loan.id}`)}
                              style={s.viewBtn}
                            ...
                            {loan.status === "pending" ? (
                              <button
                                onClick={() => router.push(`/dsa/loans/${loan.id}/edit`)}
                                style={s.editBtn}
        ...
        {loans.length > 0 && (
          <div className="dsa-pagination-row" style={s.paginationRow}>
            <button
