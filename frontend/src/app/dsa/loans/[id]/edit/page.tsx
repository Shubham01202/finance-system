// Path: frontend/src/app/dsa/loans/[id]/edit/page.tsx
"use client";
...
export default function DSAEditPage() {
  ...
  const [dsaInfo, setDsaInfo]   = useState({ name: "", firm: "" });
  ...
  const fetchApplication = async (token: string) => {
    try {
      ...
      console.log("[DSA EditPage] full application response:", data);
      ...
      if (data.success) {
        ...
        console.log("[DSA EditPage] raw documents field:", rawDocs);

        const normalized = normalizeDocuments(rawDocs);
        console.log("[DSA EditPage] normalized documents:", normalized);
        setExistingDocuments(normalized);

        if (app.status !== "pending") {
          router.push(`/dsa/loans/${id}`);
          return;
        }

        setDsaInfo({ name: app.dsa_name || "", firm: app.dsa_firm || "" });
      ...
      console.error("[DSA EditPage] fetchApplication error:", err);
  ...
  const openDoc = (url: string) => {
    ...
    try {
      ...
      console.error("[DSA EditPage] openDoc error:", err);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        setDocuments({});
        setTimeout(() => router.push(`/dsa/loans/${id}`), 1500);
      } else {
        setError(data.message || "Update failed.");
      }
    } catch (err) {
      console.error("[DSA EditPage] handleSubmit error:", err);
  ...
  return (
    <DSALayout s={s} userName={userName} handleLogout={handleLogout}>
      <div className="dsa-edit-wrap">

        {/* Top Bar */}
        <div style={s.topBar}>
          <button style={s.backBtn} onClick={() => router.push(`/dsa/loans/${id}`)}>
        ...
        {/* DSA Filing Info Banner */}
        <div style={s.dsaNotice}>
          <FaUserTie size={14} color="#92400e" style={{ flexShrink: 0 }} />
          <div style={{ fontSize: 13, color: "#92400e" }}>
            <strong>DSA Filing Mode</strong> — Changes made by <strong>{dsaInfo.name || userName}</strong>
            {dsaInfo.firm ? ` · ${dsaInfo.firm}` : ""}. Your DSA details remain linked to this application.
          </div>
        </div>
        ...
        {loading ? (
          ...
          <div style={s.formCard} className="dsa-form-card">
            ...
            <div className="dsa-submit-row" style={s.submitRow}>
              <button style={s.cancelBtn} onClick={() => router.push(`/dsa/loans/${id}`)}>
                Cancel
              </button>
...
const s: Record<string, React.CSSProperties> = {
  ...
  dsaBadge:  { display: "flex", alignItems: "center", gap: 7, background: "rgba(251,191,36,0.15)", border: "1px solid rgba(251,191,36,0.3)", borderRadius: 8, padding: "6px 12px", marginBottom: 16 },
  dsaBadgeText: { color: "#fbbf24", fontSize: 12, fontWeight: 700, letterSpacing: "0.05em" },
  ...
  dsaNotice: { display: "flex", alignItems: "center", gap: 10, background: "#fffbeb", border: "1px solid #fde68a", borderRadius: 10, padding: "12px 16px", marginBottom: 12, fontSize: 13, flexWrap: "wrap" as const },
