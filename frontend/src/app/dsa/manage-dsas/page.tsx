...
/* ─── Main Page ─────────────────────────────────────────── */
export default function ManageDsaPage() {
  const router = useRouter();
  const { toasts, showToast, removeToast } = useToast();

  const [dsas, setDsas] = useState<DSAUser[]>([]);
  ...
  const fetchDsas = async () => {
    try {
      setLoading(true);
      const res = await apiFetch("/dsa/created");
      if (res.success) setDsas(res.data);
    } catch (err) {
      console.error(err);
      showToast("Failed to load DSAs.", "error");
    } finally {
      setLoading(false);
    }
  };
  ...
  const createDsa = async () => {
    ...
    if (setupMethod === "manual" && (!password || password.length < 6)) {
      showToast("Please set a password with at least 6 characters", "error");
      return;
    }

    try {
      setSaving(true);
      const res = await apiFetch("/dsa/create", {
        method: "POST",
        body: JSON.stringify({
          full_name: fullName,
          email,
      ...
      if (res.success) {
        showToast(res.message || "DSA created successfully.", "success");
        setShowModal(false);
        resetForm();
        fetchDsas();
      } else {
        showToast(res.message || "Failed to create DSA.", "error");
      }
    } catch (err) {
      console.error(err);
  ...
  const verifiedCount = dsas.filter((d) => d.is_verified).length;
  const activeCount = dsas.filter((d) => d.is_active).length;

  /* ══════════════════════ STYLES ════════════════════════
     Includes BOTH the keys DSALayout/DSASidebar need (page, sidebar,
     logo*, caBadge*, nav*, sidebarUser, avatarCircle, userInfo,
     userName, userRole, logoutBtn, main) AND this page's own
     content styles — DSALayout/DSASidebar are used unmodified.
  ═══════════════════════════════════════════════════════════ */
  const s: Record<string, React.CSSProperties> = {
    /* ---- layout / sidebar (consumed by DSALayout + DSASidebar) ---- */
    page: {
      display: "flex",
      minHeight: "100vh",
      background: "#f1f5f9",
  ...
  return (
    <DSALayout s={s} userName={userName} handleLogout={handleLogout}>
      ...
      {/* Top bar */}
      <div style={s.topBar} className="mdsa-topbar">
        <div>
          <h1 style={s.pageTitle}>
            <FaUserTie /> Manage DSAs
          </h1>
          <div style={s.pageSubtitle}>Manage Direct Selling Agents</div>
        </div>

        <button style={s.addBtn} onClick={() => setShowModal(true)}>
          <FaPlus size={12} /> Add DSA
        </button>
      </div>
      ...
      <div style={s.statsRow}>
        {[
          { label: "Total DSAs", value: dsas.length, color: "#3b82f6" },
      ...
      <div style={s.tableWrap}>
        {loading ? (
          <div style={s.loadingWrap}>Loading DSAs…</div>
        ) : (
          <>
            <div className="mdsa-table-wrap" style={{ overflowX: "auto" }}>
              <table style={s.table}>
                <thead style={s.thead}>
                  <tr>
                    <th style={s.th}>#</th>
                      ...
                      <tr key={dsa.id}>
                        ...
                        <td style={{ ...s.td, fontWeight: 600, color: "#0f172a" }}>
                          <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
                            <Avatar name={dsa.full_name} />
                            {dsa.full_name}
                          </div>
                        </td>

                        <td style={s.td}>{dsa.email}</td>
                        ...
                        <td style={s.td}><VerifiedBadge verified={dsa.is_verified} /></td>
                        <td style={s.td}><PasswordBadge created={dsa.password_created} /></td>
                        <td style={s.td}><ActiveBadge active={dsa.is_active} /></td>
                        <td style={s.td}>{new Date(dsa.created_at).toLocaleDateString()}</td>
                      </tr>
                    ))
                  ) : (
                    <tr>
                      <td colSpan={8} style={{ ...s.td, textAlign: "center" as const, padding: "48px", color: "#94a3b8" }}>
                        ...
                        <div>{search ? "No DSAs match your search." : "No DSAs found."}</div>
                      </td>
                    </tr>
                  )}
                </tbody>
            ...
            {/* Mobile card list */}
            <div className="mdsa-card-list">
              {paginated.length > 0 ? (
                paginated.map((dsa) => (
                  <div key={dsa.id} className="mdsa-dsa-card">
                    <div className="mdsa-dsa-card-top">
                      <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
                        <Avatar name={dsa.full_name} />
                        <div>
                          <div style={{ fontWeight: 600, color: "#0f172a", fontSize: "14px" }}>{dsa.full_name}</div>
                          <div style={{ color: "#64748b", fontSize: "12.5px" }}>{dsa.email}</div>
                        </div>
                      </div>
                    </div>

                    <div className="mdsa-dsa-card-badges">
                      <VerifiedBadge verified={dsa.is_verified} />
                      <ActiveBadge active={dsa.is_active} />
                      <PasswordBadge created={dsa.password_created} />
                    </div>

                    <div className="mdsa-dsa-card-row">
                      <span className="mdsa-dsa-card-label">Mobile</span>
                      <span>
                        {dsa.mobile ? (
                          <span style={{ display: "inline-flex", alignItems: "center", gap: "6px" }}>
                            <FaPhoneAlt size={10} style={{ color: "#94a3b8" }} />
                    ...
                    <div className="mdsa-dsa-card-row">
                      <span className="mdsa-dsa-card-label">Created</span>
                      <span>{new Date(dsa.created_at).toLocaleDateString()}</span>
                    </div>
                  </div>
                ))
              ) : (
        ...
        >
          <div style={s.modal} onClick={(e) => e.stopPropagation()}>
            <div style={s.modalHeader}>
              <h2 style={s.modalTitle}>Add New DSA</h2>
              <FaTimes
            ...
            <div style={{ display: "flex", gap: "10px", justifyContent: "flex-end", marginTop: "10px" }}>
              ...
              <button style={s.submitBtn} disabled={saving} onClick={createDsa}>
                {saving ? "Creating..." : "Create DSA"}
              </button>
            </div>
          </div>
        </div>
