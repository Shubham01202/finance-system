// Path: frontend/src/app/dsa/dashboard/page.tsx
"use client";
...
export default function DSADashboardPage() {
  ...
  const fetchDashboard = async (token: string) => {
    try {
      ...
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/dsa/dashboard`, {
        headers: { Authorization: `Bearer ${token}` },
      });

      if (res.status === 401) { localStorage.removeItem("token"); router.push("/"); return; }
      if (res.status === 400) { router.push("/dsa/profile/setup"); return; }
      ...
      if (data.success) {
        ...
        if (data.recent?.[0]?.dsa_agency) setAgencyName(data.recent[0].dsa_agency);
      } else {
  ...
  return (
    <DSALayout s={s} userName={userName} handleLogout={handleLogout}>
      ...
      {/* Top Bar */}
      <div style={s.topBar} className="dsa-topbar">
        <div>
          <h1 style={s.greeting}>Welcome back, {userName.split(" ")[0]} 👋</h1>
          <div style={s.greetingSub}>
            {agencyName ? `${agencyName} · ` : ""}DSA Dashboard Overview
          </div>
        </div>
        <button style={s.newBtn} onClick={() => router.push("/dsa/apply")}>
          + New Application
        </button>
      </div>
      ...
      {/* ── STATS GRID ── */}
      <div style={s.statsGrid} className="dsa-stats-grid">
      ...
      {/* ── RECENT APPLICATIONS ── */}
      <div style={s.card} className="dsa-card">
        <div style={s.cardHead}>
          <h2 style={s.cardTitle}>Recent Applications</h2>
          <button style={s.viewAllBtn} onClick={() => router.push("/dsa/loans")}>
            View All →
          </button>
        </div>
        ...
        {loading ? (
          ...
          <>
            ...
            <div className="dsa-table-wrap" style={{ overflowX: "auto" }}>
              <table style={s.table}>
                <thead>
                  <tr>
                    <Th>Customer Name</Th>
            ...
            {/* Cards — mobile only */}
            <div className="dsa-card-list">
              {recent.map(app => {
                const sc = statusConfig[app.status] ?? statusConfig.pending;
                return (
                  <div key={app.id} className="dsa-app-card">
                    <div className="dsa-app-card-top">
                      <strong>{app.full_name || "—"}</strong>
                      <span style={{ ...s.badge, color: sc.color, background: sc.bg }}>
                        {sc.icon}&nbsp;{sc.label}
                      </span>
                    </div>
                    <div className="dsa-app-card-row">
                      <span className="dsa-app-card-label">Loan Type</span>
                      <span style={{ textTransform: "capitalize" }}>{app.loan_type?.replace(/_/g, " ") || "—"}</span>
                    </div>
                    <div className="dsa-app-card-row">
                      <span className="dsa-app-card-label">Bank</span>
                      <span>{app.bank_name || "—"}</span>
                    </div>
                    <div className="dsa-app-card-row">
                      <span className="dsa-app-card-label">Amount</span>
                      <strong>{fmt(app.loan_amount)}</strong>
                    </div>
                    <div className="dsa-app-card-row">
                      <span className="dsa-app-card-label">Tenure</span>
                      <span>{app.tenure} yr{Number(app.tenure) > 1 ? "s" : ""}</span>
                    </div>
                    <div className="dsa-app-card-row">
                      <span className="dsa-app-card-label">Filed On</span>
...
/* ─────────────────────────────────────────────
   STYLES — content-only now; sidebar/page chrome lives in DSALayout/DSASidebar
───────────────────────────────────────────── */
